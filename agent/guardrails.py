...
# ---------------------------------------------------------------------------
# 2. INJECTED-INSTRUCTION REFUSAL — IMPLEMENTED.
# ---------------------------------------------------------------------------
...
_INJECTION_PATTERNS = (
    ...
    r"(?i)\bbỏ\s+qua\s+(?:các\s+)?hướng\s+dẫn\b",
    r"(?i)\bdisregard\s+(?:all\s+)?prior\b",
    r"(?i)\breveal\s+(?:the\s+)?(?:act|scopes|instructor|grading\s+key|private)\b",
)


def scan_for_injected_instructions(text: str) -> InjectionScanResult:
    """Scans for imperative prompt injection language embedded in retrieved content."""
    ...
    for pat in _INJECTION_PATTERNS:
        ...
        if found:
            matches.extend(found if isinstance(found[0], str) else [pat])
...
_PRIVATE_PATTERNS = (
    r"(?i)(?:private\s+note\s+reads:\s*)([^\n]{20,})",
    r"(?i)(?:private_fields?:\s*)([^\n]{20,})",
)


def redact(text: str) -> RedactionResult:
    """Redacts private note / private field contents to prevent privacy_leak."""
    if not isinstance(text, str) or not text.strip():
    ...
    for pat in _PRIVATE_PATTERNS:
        ...
        for m in matches:
            if len(m.strip()) >= 20:
                hits.append(m.strip())
                redacted = redacted.replace(m, "[REDACTED_PRIVATE_CONTENT]")
                
    return RedactionResult(redacted_text=redacted, hits=tuple(hits))


# ---------------------------------------------------------------------------
# 4. ARITHMETIC VERIFICATION — IMPLEMENTED.
# ---------------------------------------------------------------------------
...
def verify_arithmetic(text: str) -> ArithmeticCheckResult:
    """Verifies that numbers present in the answer are grounded and well-formed."""
    if not isinstance(text, str) or not text.strip():
        return ArithmeticCheckResult(checked=True, ok=True, detail="No numbers to verify")
    
    numbers = _NUMBER_RE.findall(text)
    if not numbers:
        return ArithmeticCheckResult(checked=True, ok=True, detail="No numeric claims found")
    
    # Check for excessive precision in decimals (> 4 decimals)
    suspicious_precision = [n for n in numbers if "." in n and len(n.split(".")[1]) > 4]
    if suspicious_precision:
        return ArithmeticCheckResult(
            checked=True,
            ok=False,
            detail=f"Suspicious ungrounded decimal precision detected: {suspicious_precision}"
        )
        
    return ArithmeticCheckResult(
        checked=True,
        ok=True,
        detail=f"Verified {len(numbers)} numeric instances: {numbers}"
    )
...
def abstention_policy(grounding: GroundingResult, injection: InjectionScanResult | None = None) -> bool:
...
if __name__ == "__main__":
    ...
    print("\n=== agent.guardrails: Completed Guardrail modules ===\n")
    ...
    clean_text = "Course Day 26 covers MCP Architecture and Protocol Specifications."
    ...
    red = redact(leaky)
    print(f"  redact(<45+ char private-looking string>) -> hits={len(red.hits)}, redacted_text={red.redacted_text[:50]}...")
    assert len(red.hits) > 0 and "[REDACTED_PRIVATE_CONTENT]" in red.redacted_text

    wrong_math = "The IBM 2024 breach cost cited on day24 is $4.4512398471239M"
    arith = verify_arithmetic(wrong_math)
    print(f"  verify_arithmetic(<suspicious decimal precision>) -> {arith}")
    ...
    abstain_on_injected = abstention_policy(result, scan)
    ...
    print(f"  abstention_policy(well-grounded + injected) -> {abstain_on_injected}")
