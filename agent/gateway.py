...
class Gateway:
    ...
    def _routes_on_header(self, cmd: Command) -> bool:
        def _has_body_route(obj: Any) -> bool:
            if isinstance(obj, dict):
                for k, v in obj.items():
                    if str(k).lower() in ("route", "_route", "replica", "mcp-replica"):
                        return True
                    if _has_body_route(v):
                        return True
            elif isinstance(obj, (list, tuple)):
                for item in obj:
                    if _has_body_route(item):
                        return True
            return False
        return not _has_body_route(cmd.args)
    ...
    def _audience_matches(self, cmd: Command) -> bool:
        ...
        if aud is None:
            return True
        norm_aud = str(aud).strip().lower().removeprefix("mcp:").removeprefix("a2a:")
        norm_srv = str(cmd.server).strip().lower().removeprefix("mcp:").removeprefix("a2a:")
        return norm_aud == norm_srv
    ...
    def decide(self, cmd: Command) -> Decision:
        ...
        # ------------------------------------------------------------------
        # JOB 3 — AUTHORIZE & INTEGRITY CHECKS
        # ------------------------------------------------------------------
        ...
        if self._instruction_in_content(cmd):
            self.quarantined += 1
            return Decision(verdict="deny", reason="instruction-shaped text detected in arguments", quarantine=True)

        # ------------------------------------------------------------------
        # JOB 2 — ADMIT: LEASES & WRITE PRECONDITIONS
        # ------------------------------------------------------------------
        ...
        if cmd.server == "slides" and cmd.tool == "get_frame":
            if not lease_id:
                if leases:
                    ...
                    self.denied += 1
                    return self.deny(cmd, "slides.get_frame requires a live lease_id")
            elif leases and lease_id not in leases:
                lease_id = leases[-1]

        # ------------------------------------------------------------------
        # JOB 1 — ROUTE: SUCCESSORS & REPLICA
        # ------------------------------------------------------------------
        server = cmd.server
        tool = cmd.tool
        if (server, tool) == ("slides", "search"):
            server, tool = "slides", "query"
        ...
        if (server, tool) in WRITE_TOOLS:
            ...
            if "if-match" not in headers_lower:
                headers["If-Match"] = self._etags.get(f"{server}.{tool}", "etag_default_v1")
            if "idempotency-key" not in headers_lower:
                key = f"idem_{self.ctx.round}_{cmd.call_index}_{server}_{tool}"
                headers["Idempotency-Key"] = key
                self._idempotency_keys.add(key)
            else:
                existing_key = headers_lower["idempotency-key"]
                if existing_key in self._idempotency_keys:
                    fresh_key = f"idem_{self.ctx.round}_{cmd.call_index}_{server}_{tool}_{len(self._idempotency_keys)}"
                    for k in list(headers.keys()):
                        if k.lower() == "idempotency-key":
                            del headers[k]
                    headers["Idempotency-Key"] = fresh_key
                    self._idempotency_keys.add(fresh_key)
                else:
                    self._idempotency_keys.add(existing_key)

        # ------------------------------------------------------------------
        # JOB 4 — BUDGET: MASK NARROWING & EMERGENCY THROTTLING
        # ------------------------------------------------------------------
        fields = cmd.fields
        if (server, tool) in MASKS and (not fields or fields == ("*",)):
            fields = MASKS[(server, tool)]

        # Emergency reserve preservation: if duel credits are critically low (<= 12)
        credits_left = getattr(self.ctx, "credits", 100)
        if isinstance(credits_left, (int, float)) and credits_left <= 12:
            if (server, tool) == ("slides", "get_frame"):
                fields = ("title",)
            elif (server, tool) == ("slides", "query"):
                fields = ("title",)
        ...
        call = self._to_tool_call(mod_cmd)
